...
user='root', #usuario
passwd=None, #vazio
db='usuarios_unbot') #nome do banco de dados
...
API_TOKEN = '1275291244:AAFQz-QR1eJwM6L5vm35pzjMRPSFlBfOjVc' #@botfather
...
def send_welcome(message):
	msg = bot.reply_to(message,"Tudo bem? Este é o Unbot")#inserindo message
	cid = message.chat.id
	bot.send_message(cid,"Nosso id é: " + str(cid) + " Antes de começar, me responda: \nQual seu nome?")
...
def process_age_step(message):
	try:
		...
		markup.add('aeroespacial','software') #quais as categorias
		msg = bot.reply_to(message, 'Qual é o seu curso na FGA?',reply_markup=markup) #envia a opcao
...
def process_team_step(message):
	try:
		...
		team = message.text
		user = user_dict[chat_id]
		if (team == u'aeroespacial') or (team == u'software'):
			user.team = team
		else:
			raise Exception()
...
def process_mail_step(message):
	try:
		...
		# nome da base
		cur.execute("USE usuarios_unbot") #executando base a ser usada
